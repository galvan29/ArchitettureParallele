...
{ 
  ...
  for(int i = 0; i< letterali; i++){
    ...
    if((sol[i]==0 && sol[i+letterali]==0) || riprendoSoluzione){
      if(!riprendoSoluzione){
        ...
        prox[0].push_back(trasformaDaArrayAInt(sol_backup, nTotLet));
      }
    ...
    cout<<endl<<"Possibile soluzione :"<<endl;
    for(int ssif = 0; ssif < nTotLet; ssif++){
      cout<<sol[ssif]<<" ";
    }
    soluzioniRegistrate[0].push_back(trasformaDaArrayAInt(sol, nTotLet));
    cout<<endl;
    if(prox[0].size() > 0)
