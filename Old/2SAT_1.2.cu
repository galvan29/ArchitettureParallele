...
{ 
  ...
  BigInt big1 = nTotLet;
  big1 = big1 * big1;
  long int nTotLetx2 = big1.to_long();
  ...
  gpuErrchk(cudaMalloc(&d_matrix, nTotLetx2*sizeof(bool)));
  gpuErrchk(cudaMalloc(&d_matrix2, nTotLetx2*sizeof(bool)));
  gpuErrchk(cudaMalloc(&d_matrix3, nTotLetx2*sizeof(bool)));
