	...
	for (int cont = 0; cont < sizeAdj; cont++)
	{
		...
		{ 
			...
			{ 
				...
				if(adj_matrix[pos] != 1){
					if (adj_matrix[i] && ((i % nNegPosLit) + 1))// != (primo + 1))
					{    
						adj_matrix[pos] = 1;
...
void checkNewSol(int *sol, int *solFinali, int nNegPosLit, int indexSol, int *status){         //TODO VERIFICARE
	for (int cont = 0; cont < nNegPosLit; cont++)
	{
		if(cont < indexSol)
		{
