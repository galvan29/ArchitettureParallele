...
int main(int argn, char *args[])	//main
{
	...
	if (checkDiagonal(adj_matrix, nNegPosLit, sol)){ //check if the pair of positive and negative is present. If 1 1 and -1 -1 is present, there isn't solution
		...
		do { 	continua = false;
			do { 	memset(status, 0, 3* sizeof(int));	//set status to 0
				...
				if (resumeSolution)
				{
					...
					completeSol(sol, nNegPosLit, littExist);	//if a literal has 1 and its sibling 0, I do -1 and vice versa. In order to complete the solution
					if (status[0] == 0)
					{
			...
			if (status[1] == 0)
			{
				...
				if (indexSol > 0)
				{
					checkNewSol(sol, finalSol, nNegPosLit, indexSol, status);	//check if the found solution already exists 
				}
		...
		ofstream myfileD;
		myfileD.open("durationCPU.txt", std::ios_base::app);	//save duration in durationCPU.txt
		...
		ofstream myfile;
		myfile.open("soluzioni/CPUsol" + nomeFile.substr(8));	//save solution in soluzioni/.txt
