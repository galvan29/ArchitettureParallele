...
{
	int nBlock = 40, nThread = 1024;	//default value
	bool arg1 = false;
	bool arg2 = false;
	...
	for (int i = 1; i <= nConstr; i++)	//marks the existence of a litteral inside the constraints
	{
	...
	int status[3] = { 0 };
	//array for status: 0 for add to adj; 1 for -1 and 1 to the same litteral, 2 for the check of similar solution
	int *d_status;
	...
	if (status[0] == 0)
	{
		...
		do {
			...
			cudaDeviceSynchronize();
			do { 	memset(status, 0, 3* sizeof(int));	//set status to 0
				cudaMemcpy(d_status, status, 3* sizeof(int), cudaMemcpyHostToDevice);	//copy to gpu

				if (sol[i] == 0 && sol[i + nLitt] == 0 && resumeSolution == false)
				{
					//start, if two sibling literals both have 0 i.e. no value.
					memcpy(alternativeSol, sol, nNegPosLit* sizeof(int));	//copy current solution

					if (littExist[i])
					{
						//give -1 to solution and 1 to alternative solution
						sol[i] = -1;
						alternativeSol[i] = 1;
						esiste = true;
					}
				...
				if (resumeSolution)
				{
					...
					if (status[1] == 1)
					{
						//check if there is some conflict
						break;
					}
				}
			} while (i < nLitt);
			...
			if (status[1] == 0)
			{
				...
				if (indexSol > 0)
				{
					checkNewSol <<<nBlock, nThread>>> (d_sol, d_finalSol, nNegPosLit, indexSol, d_status);	//check if the found solution already exists 
					cudaDeviceSynchronize();
				}
		...
		ofstream myfile;
		myfile.open("soluzioni/sol" + nomeFile.substr(8));	//save solution in soluzioni/.txt
		...
		myfile.close();
		cout << "Execution finished. " << indexSol << " solutions of the " << k+indexSol << " searched have been found." << endl;
	}

	else
	{
		cout << "Conflict between constraints, there are no solutions." << endl;
	}
